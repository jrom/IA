...
public class Laberinto extends javax.swing.JPanel {
                ...
                for (int j = 0; j < nc; j++) {
                    if (plano[i][j] != 0) {
                        if (plano[i][j] < 0) {
                            ...
                            g2.fill(rect);
                            g2.setPaint(Color.black);
                            double area = (double) plano[i][j];
                            if (area > (sep * sep)) {
                                area = sep * sep;
                            }
                            area = Math.sqrt(area);
                            Rectangle2D rect2 = new Rectangle2D.Double((i * sep) + gapX, (j * sep) + gapY, area, area);
                            g2.fill(rect2);
    ...
    public void testPlano(int x, int y) {
    ...
    public void changeState(double x, double y) {
        ...
        if (i >= 0 && i < nc && j >= 0 && j < nc) {
            if (plano[i][j] == -1) {
                plano[i][j] = 0;
            } else if (plano[i][j] == 0 || plano[i][j] == -4) {
                plano[i][j] = -1;
            }
            repaint();
    ...
    private void updateTest(int i, int j) {
